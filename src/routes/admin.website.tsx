...
function UploadZone({ label, hint, accept, maxMB, aspect, currentUrl, onFile, readOnly }: {
  ...
  // Read-only: hanya tampilkan thumbnail
  if (readOnly) {
    return (
...
function WebsiteCMS() {
  ...
  const waWarn = waNumber && !/^(08|\+62)\d{8,11}$/.test(waNumber.replace(/[-\s]/g, ""))
    ? "Nomor tidak valid. Contoh: 0878-6391-2739" : undefined;
  const mapsWarn = mapsUrl && !/(maps\.google|goo\.gl\/maps|maps\.app\.goo\.gl)/.test(mapsUrl)
    ? "Bukan link Google Maps — pastikan URL benar." : undefined;
  ...
  async function saveHeader() {
    ...
    try {
      ...
      toast("Header logo updated successfully.");
    } catch { toast("Failed to save. Please try again.", false); }
  ...
  async function saveFooterLogo() {
    ...
    try {
      ...
      toast("Footer logo updated successfully.");
    } catch { toast("Failed to save. Please try again.", false); }
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="space-y-3">
        ...
        <Accordion title="Header" open={open === "header"} onToggle={() => toggle("header")}>
          <UploadZone label="Upload New Logo" hint="PNG, SVG, JPG · Max 2 MB · Min 200×200px · Transparent background recommended"
        ...
        <Accordion title="Footer" open={open === "footer"} onToggle={() => toggle("footer")}>
          ...
          <SubSection title="3a — Footer Logo">
            <UploadZone label="Upload Footer Logo" hint="PNG, SVG · Gunakan versi putih/terang (background footer adalah gradient)"
